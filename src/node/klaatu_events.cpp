...
float touch_scaling;
inline int32_t scalep(int32_t data, RawAbsoluteAxisInfo *axisp)
{
    ...
    for (size_t index = 0; index < event_indication; index++ ) {
        ...
        if (p->type == EventHubInterface::DEVICE_ADDED) {
            ...
            if (deviceclass & INPUT_DEVICE_CLASS_TOUCH_MT)
                ...
                touch_scaling = float(display_w + display_h) / float(
                     touchsensor_axis_x.maxValue - touchsensor_axis_x.minValue
                     + touchsensor_axis_y.maxValue - touchsensor_axis_y.minValue);
                printf("new touch: %s\n", name);
            }
            ...
            if (p->CODE_FIELD == ABS_MT_POSITION_X)
                current_point->x = scalep(p->value, &touchsensor_axis_x);
            else if (p->CODE_FIELD == ABS_MT_POSITION_Y)
                current_point->y = scalep(p->value, &touchsensor_axis_y);
            ...
            else if (p->CODE_FIELD == ABS_MT_PRESSURE)
